...
export class Firestore {
  ...
  async guardarMensaje(mensaje: MensajeChat): Promise<void> {
    try {
      ...
      const coleccionMensajes = collection(this.firestoreService, 'mensajes');
  ...
  obtenerMensajesUsuario(usuarioId: string): Observable<MensajeChat[]> {
    return new Observable((observer) => {
      ...
      const consulta = query(
        collection(this.firestoreService, 'mensajes'),
  ...
  async guardarConversacion(conversacion: ConversacionChat): Promise<void> {
    try {
      const coleccionConversaciones = collection(this.firestoreService, 'conversaciones');
